...
## margem de erro = diferenÁa do intervalo de confianÁa / 2
...
## H· evidÍncia que n„o houve mudanÁa. visto que o intervalos tem uma grande interseÁ„o
...
## H· evidÍncia que houve mudanÁa. visto que o intervalos n„o tem interseÁ„o
...
0.05 * 39
## R. Erro do tipo I consiste em, num testes de hipÛteses, rejeitar a hipÛtese nula quando ela È verdadeira.
## Ao fixar um nÌvel de signific‚ncia de 5% num determinado teste, 
## estamos a afirmar que em 5% das vezes rejeitaremos a hipÛtese nula sendo esta verdadeira.
## Ou seja, sendo o nÌvel de signific‚ncia 0.05 para os 39 paÌses, podemos esperar que o erro
## aconteÁa para 0.05 * 39 = 1.95 casos.
...
## revisitando relaÁ„o entre p e me
n <- 1000
p <- seq(0, 1, 0.01)
me <- 1.96*sqrt(p*(1 - p)/n)
plot(me ~ p) # relaÁ„o me & p
 
## me = 0.01
## nÌvel de confianÁa 0.95
## me = 1.96 * ep; ep = sqrt(p*(1 - p)/n)
## transformando fÛrmula em funÁ„o de N
## me <- 1.96 * 2*sqrt(p*(1 - p)/n) transaformar fÛrmula em funÁ„o de n
## me^2 <- (1.96^2 * p * (1-p))/N
## N <- (1.96^2 * p * (1-p))/me^2
p <-seq(0, 1, 0.01)
me <-0.01
N<- 1.96^2 * p * (1-p)/me^2
plot(N ~ p) # relaÁ„o N & p
...
## Participantes m·ximo para a confianÁa desejada È 9604
